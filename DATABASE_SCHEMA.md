...
1. **查询优化**：建议使用创建好的索引字段进行查询
2. **关联查询**：事实表与维度表通过外键关联
3. **数据分析**：时间维度表支持各种时间粒度分析
4. **数据更新**：注意维护外键关系的完整性
...
- **金额字段**：单位为元（人民币），精确到小数点后2位
- **数量字段**：单位为件
- **时间字段**：日期为YYYY-MM-DD格式，时长为秒
- **百分比字段**：0.10表示10%的折扣
...
将以上内容保存为 `DATABASE_SCHEMA.md` 文件即可直接用于GitHub项目。